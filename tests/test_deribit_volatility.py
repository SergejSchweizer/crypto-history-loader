...
def test_fetch_historical_volatility_range_parses_rows(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    def _fake_get_json(url: str, params: dict[str, object]):
        assert "get_historical_volatility" in url
        assert params["currency"] == "BTC"
        return {"result": [[1000, 12.3], [2000, 14.0], [2000, 13.5], [3000, 15.0]]}

    monkeypatch.setattr(deribit_volatility, "get_json", _fake_get_json)
    rows = deribit_volatility.fetch_historical_volatility_range(currency="btc", start_open_ms=1000, end_open_ms=2500)
    assert rows == [
        {"timestamp": 1000, "volatility": 12.3},
        {"timestamp": 2000, "volatility": 13.5},
    ]
