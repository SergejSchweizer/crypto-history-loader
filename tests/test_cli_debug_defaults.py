...
def test_non_build_commands_keep_debug_logging_opt_in() -> None:
    ...
    assert _is_debug_logging_enabled(argparse.Namespace(command="list-spot_ohlcv-timeframes", debug=False)) is False
    assert _is_debug_logging_enabled(argparse.Namespace(command="list-spot_ohlcv-timeframes", debug=True)) is True
