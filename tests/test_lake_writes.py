...
def _spot_row(open_time: datetime, close: float) -> dict[str, object]:
    return {
        "exchange": "deribit",
        "dataset_type": "spot",
...
def test_merge_and_deduplicate_rows_keeps_latest_record_and_sorts_by_open_time() -> None:
    ...
    merged = merge_and_deduplicate_rows(
        existing=[_spot_row(open_time=second_time, close=101.0), _spot_row(open_time=first_time, close=100.0)],
        new=[_spot_row(open_time=first_time, close=102.0)],
    )
...
def test_write_partition_file_rewrites_existing_partition_without_duplicate_keys(tmp_path: Path) -> None:
    ...
    key = ("deribit", "spot", "BTCUSDT", "1m", "2026-04-27")
    first_time = datetime(2026, 4, 27, 10, 0, tzinfo=UTC)
    ...
    first_path = write_partition_file(
        ...
        run_id="run-1",
        key=key,
        rows=[_spot_row(open_time=first_time, close=100.0)],
    )
    second_path = write_partition_file(
        ...
        run_id="run-2",
        key=key,
        rows=[_spot_row(open_time=first_time, close=101.0), _spot_row(open_time=second_time, close=102.0)],
    )

    expected_path = partition_path(lake_root=lake_root, dataset_type="spot", key=key) / "data.parquet"
