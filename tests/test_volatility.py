...
def test_fetch_historical_volatility_range_parses_points(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    def _fake_fetch_range(**kwargs: object):
        assert kwargs["currency"] == "BTC"
        return [{"timestamp": 1_000, "volatility": 12.5}]

    monkeypatch.setattr("ingestion.volatility.deribit_volatility.fetch_historical_volatility_range", _fake_fetch_range)
    rows = fetch_historical_volatility_range(
        exchange="deribit",
    ...
    assert len(rows) == 1
    assert rows[0].dataset_type == "historical_volatility"
...
def test_fetch_volatility_index_range_parses_points(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    def _fake_fetch_range(**kwargs: object):
        ...
        return [{"timestamp": 2_000, "index_value": 75.2}]
