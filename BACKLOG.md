...
repository review) and takes priority over the Refactor Architecture Stack below, beginning with PR-47. The
Refactor Architecture Stack should resume only after QC-01 through QC-03 are merged and affected Silver and
Gold artifacts have a documented rebuild plan. The existing architecture-refactor stack may proceed in parallel
only where it cannot change, obscure, or lock in the affected quantitative behavior. Data correctness takes
precedence over structural cleanup.
