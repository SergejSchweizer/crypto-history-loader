"""Funding symbol-level Bronze fetch planning."""
...
def fetch_symbol_funding(
    ...
    if tail_delta_only:
        ...
        if start_open_ms > end_open_ms:
            return []
        # Funding is naturally sparse, so a single ranged call avoids re-paging
        # the same backend data through many day-sized requests.
